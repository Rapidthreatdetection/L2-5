function [CM, DM] = simcolumn_connectivity_ThtoL23Conn(preCells, postCells)
% calculate connectivity matrix, based on information provided in structure
% array Neurons
% thalamus to L4 connections
...
for i = 1:length(postCells) % get DendRatio for postsynaptic cells
    try
       DendRatio(i) = postCells{i}.DendConnRatio(postCells{i}.DendriteTarget == 0);
    catch
        DendRatio(i) = 0;
    end
end
...
% in term of thalamus axons, the distance matrix is calculated as axon
% center to the postsynaptic cell
for i = 1:length(postCells)
  postc = postCells{i}.Loca;
  for j = 1:length(preCells)
    prec = preCells{j}.Axon{preCells{j}.AxonTarget == postCells{i}.Layer} (1:3);
    DistMat(i, j) =(sum((prec - postc).^2)).^0.5;
  end
end
...
for i = 1:length(PreType_Unique)
    switch PreType_Unique(i)  
        case(1) % presynaptic cell is spiny pyramidal
            CM(:, preIND(i)+1:preIND(i+1)) = simConn_ThtoL23_ThtoAll(AxonDendOverlap(:, preIND(i)+1:preIND(i+1)), ...
...
%             CM(:, preIND(i)+1:preIND(i+1)) = simConn_L4toL4_FsPVtoAll(AxonDendOverlap(:, preIND(i)+1:preIND(i+1)), ...
%                 postIND, PostType_Unique, AxonRatio(preIND(i)+1:preIND(i+1)), DendRatio);
%         case(4) % presynaptic cell is RS interneurons
%             CM(:, preIND(i)+1:preIND(i+1)) = simConn_L4toL4_RSNPtoAll(AxonDendOverlap(:, preIND(i)+1:preIND(i+1)), ...
%                 postIND, PostType_Unique, AxonRatio(preIND(i)+1:preIND(i+1)), DendRatio);
    end
end
