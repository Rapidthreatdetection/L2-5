...
Kernels.ActivationFunction.function = @activation_Petersen_2008_ND; % sigmoid activation_identity activation_Petersen_2008_ND
...
%% Curvature kernels and activation functions #Tag:输入的修改:2 针对卷积核和激活函数参数
params_ckernels.mf = 10; % 10 控制核的主频率范围
params_ckernels.spreadf = 40;
params_ckernels.mv = 3; % normal 3 激活函数参数,控制sigmoid的陡峭程度 激活函数斜率：增大 mv → 响应更陡峭、更集中
params_ckernels.spreadv = 0.2;
params_ckernels.mk = 120; % normal 120 激活函数增益参数k,放大输入 提高 PSTH 峰值：增大 mk → 放大整体 firing rate
...
if Nkernel_ba>1
    ...
    Kernels.kerneltime = (0:20)*binsize;% 时间轴定义，决定核的最大长度 
end
%% Curvature Kernels
