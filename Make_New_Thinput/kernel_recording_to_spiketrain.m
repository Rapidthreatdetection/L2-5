...
% 显示 la 和 lc 的值
disp(['Non-causal part length (la): ', num2str(la)]);
disp(['Causal part length (lc): ', num2str(lc)]);
...
% Debug: show which activation function will be used
if isfield(KernelStruct, 'ActivationFunction') && isfield(KernelStruct.ActivationFunction, 'function')
    try
        disp(['Using activation function: ', func2str(KernelStruct.ActivationFunction.function)]);
    catch
        disp('Using activation function: <unknown handle>');
    end
end
for nk = 1:Nkernel
    for nt = 1:Ntrace
...
%     end   
    ...
    maxt = 0;
    win   = [100 120];   % 时间窗口(ms)
    xtick_values = linspace(100, 120, 5);  % Original x-axis tick positions
    xtick_labels = linspace(0, 20, 5);   % New x-axis labels from 0 to 100 ms
    for nt = 1:Ntrace
        figure
        for nk = 1:Nkernel
            for ndim = 2:Ndimk
